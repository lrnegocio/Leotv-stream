    ...
    <div className="min-h-screen bg-background pb-20">
      <header className="fixed top-0 inset-x-0 h-16 bg-gradient-to-b from-background to-transparent z-50 flex items-center justify-between px-6">
        <div className="flex items-center gap-8">
          ...
          <nav className="hidden md:flex gap-6 text-sm font-medium text-muted-foreground">
            ...
            <Link href="/movies" className="hover:text-primary transition-colors">Filmes</Link>
            <Link href="/series" className="hover:text-primary transition-colors">Séries</Link>
            <Link href="/categories" className="hover:text-primary transition-colors">Canais P2P</Link>
          </nav>
        </div>
      ...
      <section className="relative h-[85vh] w-full flex items-end">
        ...
        <div className="relative z-10 p-8 md:p-16 max-w-2xl space-y-6 animate-in slide-in-from-bottom duration-700">
          <Badge className="bg-primary text-white mb-2">Destaque do Dia</Badge>
          <h1 className="text-5xl md:text-7xl font-extrabold font-headline leading-tight tracking-tighter uppercase">Rede Master P2P</h1>
          <p className="text-lg md:text-xl text-muted-foreground line-clamp-3">
            Assista aos seus canais favoritos, filmes e séries com a maior estabilidade do mercado. Qualidade 4K garantida sem travamentos.
          </p>
      ...
      <div className="px-6 md:px-16 -mt-32 relative z-20 space-y-12 pb-24">
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-bold font-headline">Adicionados Recentemente</h2>
            ...
            {mockContent.map((item) => (
              <Link key={item.id} href={`/watch/${item.id}`} className="group relative transition-all-smooth hover:scale-105">
                ...
                <div className="mt-2">
                  ...
                  <p className="text-xs text-muted-foreground">{item.genre} • {item.type === 'channel' ? 'AO VIVO' : 'Filme'}</p>
                </div>
              </Link>
            ))}
          </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-bold font-headline">Canais em Destaque</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {mockContent.filter(c => c.type === 'channel').map((item) => (
               <Link key={item.id} href={`/watch/${item.id}`} className="group relative overflow-hidden rounded-xl aspect-video transition-all-smooth hover:shadow-2xl hover:shadow-primary/20">
                <img src={item.thumbnail} alt="" className="w-full h-full object-cover brightness-[0.7] group-hover:scale-110 transition-transform duration-700" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent p-4 flex flex-col justify-end">
                   <h3 className="text-xl font-bold">{item.title}</h3>
                   <p className="text-xs text-white/60">Transmissão Direta HD</p>
                </div>
              </Link>
            ))}
          </div>
