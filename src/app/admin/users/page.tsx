...
import { Plus, Search, UserCheck, UserX, Clock, Shield, RefreshCcw, Trash2 } from "lucide-react"
...
import { mockUsers, generateRandomPin, addUser, updateUser, User, SubscriptionTier } from "@/lib/store"
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold font-headline">Gerenciar Acessos</h1>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <div className="flex gap-2">
                  <Input value={newUser.pin} readOnly placeholder="Clique em gerar" />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {newUser.tier === 'test' && (
                  <div className="grid gap-2">
                    ...
                    <Input type="number" value={newUser.hours} onChange={e => setNewUser({...newUser, hours: e.target.value})} />
                  </div>
                )}
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Input type="number" value={newUser.screens} onChange={e => setNewUser({...newUser, screens: e.target.value})} />
                </div>
                <div className="grid gap-2">
                  <Label>Senha Kids/Adulto</Label>
                  <Input maxLength={4} value={newUser.parentalPin} onChange={e => setNewUser({...newUser, parentalPin: e.target.value})} />
                </div>
              </div>
            </div>
            <DialogFooter>
              <Button onClick={handleAddUser} className="w-full">Confirmar e Gerar</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <div className="relative">
        ...
        <Input 
          placeholder="Buscar por PIN ou Email..." 
      ...
      <div className="bg-card/50 border border-white/5 rounded-xl overflow-hidden shadow-xl">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>PIN / Acesso</TableHead>
          ...
          <TableBody>
            {filteredUsers.map((user) => (
              <TableRow key={user.id}>
                <TableCell className="font-mono font-bold uppercase">{user.pin}</TableCell>
                <TableCell>
